...
if [ -d /scratch/output/default/crashes ]; then
    ...
    for crash in ./id*; do
        /fuzz_bins/bin/afl-tmin -Q -i $crash -o /scratch/output/tmin/$crash -- $CMD 
    done
    /fuzz_bins/utils/sh /fuzz_bins/bin/afl-cmin -Q -i /scratch/output/tmin/ -o /scratch/output/cmin -C -- $CMD
fi
